...
| 2   | Auth            | **Supabase Auth** (magic link primary, OAuth post-v1). Resend as Supabase's Custom SMTP so all auth emails come from `auth@mail.<domain>` with branded templates edited via Supabase dashboard. **RLS is enabled on every table** with policies declared in `packages/db/src/policies.ts`. App code (Server Actions, route handlers, worker) connects as `postgres` and bypasses RLS by design — auth is enforced in TS via `requireUser()` family. Policies are defence-in-depth: they protect direct DB access, future supabase-js usage, and Realtime. |
...
- **Auth**: Supabase Auth + `@supabase/ssr`. **RLS on**, with policies in `packages/db/src/policies.ts` applied via `pgPolicy()` in `schema.ts`. App code connects as `postgres` (BYPASSRLS) so policies are defence-in-depth, not the primary auth boundary — that's `requireUser()` / `requireCreator()` / `requireCampaignOwner()` in TS. See "Known risks" below for the trade-off and the path to full RLS-on-Drizzle enforcement.
...
- **RLS as defence-in-depth, not primary auth**: app code connects to Postgres as `postgres` (BYPASSRLS), so RLS policies don't gate Server Actions / webhook handlers / worker queries. Auth there is enforced in TS via `requireUser()` family. Policies still protect: direct psql/Studio access, future supabase-js usage from the browser, future Realtime subscriptions. To make Drizzle queries also respect RLS we'd switch to a Postgres user without BYPASSRLS and pass the user's JWT (`SET request.jwt.claims = …`) per request — meaningful change, defer until there's a concrete reason (e.g. before exposing supabase-js to the browser).
